...
public class UcenterUserController extends QController{
	...
	public void index(){
		...
		if(QPropertiesUtil.getPropertyToBoolean(QPropertiesUtil.config, "jfinal.auth.use_inside_user_page")){
			render("/WEB-INF/classes/com/uikoo9/util/jfinal/view/ucenter/ucenter-user-index.ftl");
		}else{
	...
	public void savep(){
		...
		if(QPropertiesUtil.getPropertyToBoolean(QPropertiesUtil.config, "jfinal.auth.use_inside_user_page")){
			render("/WEB-INF/classes/com/uikoo9/util/jfinal/view/ucenter/ucenter-user-input.ftl");
		}else{
